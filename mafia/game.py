...
class MafiaGame:
    ...
    async def _start(self):
        ...
        async with self.ctx.acquire() as conn:
            ...
            await conn.executemany(query, [(self.id, player.member.id) for player in winners])
            await conn.execute("UPDATE games SET day_count = $1 WHERE id = $2", self._day, self.id)
