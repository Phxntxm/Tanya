...
def _sync_make_night_image(night: int) -> io.BytesIO:
    ...
    raster.text(((1920-__w)/2, 30), f"Night {night}", font=font_28days_title, fill="black") # noqa center the night #
...
def _sync_make_day_image(game: dict, deaths: typing.List[int], avatars: dict) -> io.BytesIO:
    ...
    raster.text(((1920-__w)/2, 30), f"Day {game['d']}", font=font_28days_title, fill="black") # noqa center the day #
    ...
    def paste_avatar(player: dict, text_fill: str, do_x: bool, show_role: bool):
        ...
        if show_role:
            ...
            nick += f"\n\t{player['r']}"
            t = (x[2] + 20, int(330 + (row_width * row))) # don't bother centering as there's 2 lines of text which centers itself
